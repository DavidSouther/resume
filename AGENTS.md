...
The static site links CSS with absolute paths (`/global.css`), so open pages through a server rooted at `docs/`, never as `file://`. Serve with `node scripts/serve.ts --port 8080 --watch`, but beware that `--watch` rebuilds `docs/` from `src`. Since you're only touching `src`, this should be OK, but the loaded page will live reload. Then point the browser at `http://127.0.0.1:8080/<clean-url>` (e.g. `/blog/`). If port 8080 is already taken, a server is likely running already; reuse it rather than starting another.
