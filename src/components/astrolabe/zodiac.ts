...
export function buildZodiac(grp: SVGGElement): ZodiacRefs {
	...
	// Shaded wedges live in static SVG space (like the dividers) so their side
	// edges can be recomputed each frame to follow the guilloche curve. The arcs
	// (inner-edge highlights) stay in zwheel and rotate with the dial.
	const zwedges = g({ id: "zwedges" });
	grp.appendChild(zwedges);

	const zwheel = g({ id: "zwheel" });
	grp.appendChild(zwheel);
	...
	// Dividers live outside zwheel so their paths can be recomputed each frame
	// in static SVG space (following the guilloche curve). They start as radial
	// lines and are updated by startAnimation once Earth has moved.
