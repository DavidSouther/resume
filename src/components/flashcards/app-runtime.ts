...
/**
 * Wires up an already-built `.flashcards` app (see app.ts's
 * buildFlashcardsApp) — every browse/review interaction, both loaders of
 * this module share this one function:
 *
 *  - client.ts, for a `/flashcards/<slug>/` page whose deck is already
 *    server-rendered and just needs `getDeck` to resolve the manifest's
 *    `loadDeck(slug)`.
 *  - hub-client.ts, for a "bring your own URL" deck fetched and rendered
 *    entirely client-side, whose `getDeck` already has its DeckSource in
 *    hand and just wraps it in a resolved promise.
 *
 * `slug` namespaces this deck's progress/annotation localStorage keys —
 * only needs to be *stable for this deck*, not globally registered
 * anywhere (an external deck uses its URL as its slug; see hub-client.ts).
 */
export function mountApp(
	slug: string,
	getDeck: () => Promise<DeckSource>,
): void {
