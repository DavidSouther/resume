...
/** Pass `mount: false` to get the figure drawn but with no control bar yet. */
async function buildArtFigure(
	options: { mount?: boolean } = {},
): Promise<HTMLElement> {
	const figures =
		options.mount === false
			? render((await getPost(POST_ID)).body ?? "")
			: await mountPost();
...
describe("the keyboard", () => {
	...
	// The listing is a horizontally scrolling box, and a scroll container is
	// keyboard focusable. Stepping from there would cost the reader the arrow
	// keys they need to read a line wider than the column.
	it("leaves the arrow keys to the listing's own scrolling", async () => {
		const figure = await buildArtFigure();
		goTo(figure, 3);
		const line = figure.querySelector(".highlight-gutter-line");
		expect(line).not.toBeNull();

		key(line ?? figure, "ArrowRight");
...
describe("mounting", () => {
	// A live region that gains its text after entering the document announces.
	// A post mounts every figure at once, so reading the first step into an
	// already-inserted region would announce a step count per figure before the
	// reader has pressed anything.
	it("writes the first step before the control bar enters the document", async () => {
		const figure = await buildArtFigure({ mount: false });
		// Records are drained synchronously: a MutationObserver callback is a
		// microtask, so by the time it ran the bar would already be in place and
		// the ordering this test exists for would be unobservable.
		const observer = new MutationObserver(() => {});
		observer.observe(figure, { childList: true });

		mountTraceSteppers(figure.ownerDocument);
		...
		expect(inserted[0].querySelector('[role="status"]')?.textContent).toMatch(
			/^Step 1 of 7\b/,
		);
	});
