 ...
 *
 * Keyed on `[data-state]`, which only the stepper writes: JavaScript off,
 * before mount, on print, or after a mermaid failure, nothing here matches and
 * the diagram draws complete.
 *
 * `[data-at]` is repeated in every selector to outrank the two-class rules
 * above, where `.trace-pointer.trace-struck` would otherwise beat
 * `[data-state="future"]` and show a struck pointer before its step.
 *
 * A future item is invisible, not dim, and keeps its box.
 */
