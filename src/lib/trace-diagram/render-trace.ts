// Draws a TraceModel as SVG in the hand-drawn memory-diagram notation: a
// Name/Value T-table, value history running rightward with superseded entries
// struck, stacked call frames crossed out when complete, dashed block scopes,
// a return arrow along the left margin, and a heap area to the right.
//
// Text is measured arithmetically from a monospace metric, never with getBBox,
// so the renderer needs no layout engine and runs the same under jsdom.
...
/**
 * One entry of a value history: the value, and the strike that cancels it.
 *
 * A timed diagram wraps the label in `g.trace-value-item` so it can carry a
 * state. The strike stays a *sibling* of that group and takes the superseding
 * value's step — inside it, a strike would appear with the value it cancels
 * rather than with the one that replaces it. An untimed diagram gets neither.
 */
function valueCells(
	values: TraceValue[],
	index: number,
