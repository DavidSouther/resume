...
export interface Timed {
	...
	tag?: number;
	/**
	 * Author's `#<digits>`: which 1-based execution of `tag`'s line this is.
	 * Absent means the next unclaimed one, which is only unambiguous while a
	 * line executes once per row.
	 */
	occurrence?: number;
