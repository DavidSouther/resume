...
export function resolveSteps(model: TraceModel): StepSequence {
	...
	// How many tags have already claimed each repeated line, so a line written
	// twice in `steps` replays a loop instead of collapsing to one step.
	const taken = new Map<number, number>();
	// An untagged node shares the step of the nearest preceding tagged node
	// rather than re-resolving its line, so heap fields and a second value
	// produced by one statement never consume an occurrence of their own.
	let inherited = 1;
