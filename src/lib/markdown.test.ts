...
describe("splitHighlightedLines", () => {
	...
	// highlight.js emits only spans today, so closing by depth happened to work.
	// Closing by name means a future emitter cannot silently produce mismatched
	// tags out of markup this function never saw.
	it("closes each open element by its own name", () => {
		const lines = splitHighlightedLines("<b><i>one\ntwo</i></b>\n");

		expect(lines).toEqual(["<b><i>one</i></b>", "<b><i>two</i></b>"]);
	});
});
