...
but only one of them is code this project wrote.

**Polling — `console::read_line` (`src/console.rs`).** Reading a
keystroke is a tight loop around `EFI_SIMPLE_TEXT_INPUT_PROTOCOL::ReadKeyStroke`:
...
"Is a key ready? No. Is a key ready? No. ..." — that's the entire
input model, and it's the only I/O mode this project actually
implements. The protocol offers a way out of it (`SimpleTextInputProtocol`
carries a `wait_for_key` `EFI_EVENT`, meant for
`BootServices::WaitForEvent`), but a shell with nothing else to do while
it waits for a line has no use for that, and the brief rules out async
anyway. Console *output* (`write_str`/`write_bytes` → `OutputString`) is
the same shape from the caller's side: one blocking call, no completion
callback.

**Interrupt — happening, but never in this code.** This is the "???" the
lecture leaves for the reader. Nothing in `uefi_boot` handles an
interrupt, and it structurally can't: taking an interrupt means owning
the interrupt controller (GICv2/v3, on aarch64), which is exactly the
kind of custom device driver the brief says not to write. What this
project sees instead is the *result* of interrupt-driven work firmware
already did — a real keyboard IRQ fires, firmware's own driver services
it and buffers the keystroke, and `ReadKeyStroke` (our poll, above) just
picks that buffered result up later, whenever it happens to run. The
interrupt is real; it's just one layer further down than a UEFI
application is allowed to reach.
