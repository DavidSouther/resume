...
specialize by argument type, but the function must already be written with certain generic bounds in
...
`vec![2, 4, 6]` uses square brackets to "look like" an array literal. Rust does allow curly brackets,
but they are much less commonly used.
...
you'll sometimes need extra type hints when inference can't resolve the value. It's generally
considered bad practice to put complex logic or computation inside a declarative macro's body, and
the standard library macros don't. But the macro system is a complete grammar, so macros provided by
third-party crates may not be as well behaved.

Attribute macros wrap entire blocks, typically functions or structs, and add substantial new
functionality to their argument. Attribute macros are similar to annotations and decorators in other
languages. 
...
[`vec!`](https://doc.rust-lang.org/std/macro.vec.html) builds a `Vec<T>` in one expression instead
of creating an empty vector and pushing each element. `vec![1, 2, 3]` allocates and fills a vector
with those three elements; `vec![0; 10]` allocates a vector of ten zeros, the value before the
semicolon repeated the number of times given after it. Reach for it any time you'd otherwise write
`Vec::new()` followed by a run of `push` calls.
...
mini formatting DSL, akin to C's `printf` but without expressions or control flow like Go's `gotmpl`.

The formatting language is built around curly braces `{}` for interpolation. Without anything else,
...
to format the result of something more, use the positional arguments. These arguments are eagerly
evaluated.
...
[`log`](https://docs.rs/log/latest/log/) crate offers a comparable macro API, but structured
key-value logging is an optional feature rather than a core design point. Choosing between the `log`
and `tracing` ecosystems is largely project dependent; `tracing` is newer and more actively
developed, while `log` is simpler for basic use cases.
...
[`assert_matches!`](https://doc.rust-lang.org/std/macro.assert_matches.html) builds
on the [`matches!`](https://doc.rust-lang.org/std/macro.matches.html) macro (which returns a
boolean) by panicking and printing a `Debug` representation of the failed value. The `matches!` macro simplifies
a `match` expression: `match foo { bar => true, _ => false }` becomes `matches!(foo, bar)`.
...
lookup, e.g. `env!("PATH")`. It fails to compile if the named variable is absent on the building
machine. For runtime environment variables, [`std::env::var`](https://doc.rust-lang.org/std/env/fn.var.html)
returns a `Result` on the running machine. [`option_env!`](https://doc.rust-lang.org/std/macro.option_env.html)
allows compiling and provides an `Option` with the result of the compiling machine's environment.
...
runtime. They do, of course, increase the binary size. Game assets, or anything that needs to change
during deployment, should not be included this way.
...
is used early in development. Say you're implementing a trait for a struct, and the trait has four
...
## Common Attribute and Derive Macros

Rust's macro system has a third form alongside declarative macros: attribute macros like
`#[tokio::main]`, and derive macros like `#[derive(Debug)]`, both written as attributes on an item
rather than inline expressions. Some attributes you'll see everywhere, such as `#[test]` and
`#[cfg(...)]`, are not macros at all but built-in compiler attributes; they're grouped here because
they share the same syntax and everyday use.

### `derive`

Common traits ([Rust Book Appendix C](https://doc.rust-lang.org/book/appendix-03-derivable-traits.html))
have "obvious" implementations most of the time. Instead of making the
developer write them, [derive macros](https://doc.rust-lang.org/reference/attributes/derive.html)
...
is about the same functionality as Python's `@dataclass`. `Copy` applies only to types with no heap
allocation and no custom `Drop`. `#[derive(Error)]` from thiserror turns an
...
field of the struct isn't `Debug`, the `derive` will fail. This happens most often with
[`Default`](https://doc.rust-lang.org/std/default/trait.Default.html), since not every type has an
obvious default value; you may need to provide your own impl that chooses a correct default for a
non-default property type.
...
is explicitly testing for a panic case, though a better design can usually avoid needing it.
...
control lints. Use these to silence a lint that's too strict for a given case, or to quiet warnings
during development, not as a default habit.

[`#[tokio::main]`](https://docs.rs/tokio/latest/tokio/attr.main.html) and
[`#[tokio::test]`](https://docs.rs/tokio/latest/tokio/attr.test.html) are Tokio crate specific
