...
#let coverage-levels = (1.0, 2.0, 3.0)
...
#let coverage-diagram = lq.diagram(
  ...
  yaxis: (ticks: ((1, [1]), (3, [3]), (6, [6]), (9, [9])), subticks: none),
  grid: (stroke: guide-stroke),
  lq.plot(
...
#let multiplier-diagram = lq.diagram(
  ...
  lq.place(3.12, coverage-panel.multiplier.last().at(1), align: left, pad(left: 3pt, box(width: 1.85cm, text(size: 7pt, fill: violet)[
    log RRF, $b=1$
  ]))),
)
...
#let kernel-curve-atlas-figure() = block(width: 100%, {
  set text(fill: ink)
  grid(
