...
#let compact-chart(
  ...
  ..values.slice(0, if zero-singletons { 5 } else { values.len() }).enumerate().map(entry => {
    ...
    let label-fill = if value == 0 { ink } else if narrow { fills.at(index) } else { white }
    lq.place(label-x, rows.at(index), align: label-align, text(size: 5.5pt, weight: "bold", fill: label-fill)[#labels.at(index)])
  }),
  if zero-singletons {
    lq.place(xmin + label-offset, 6, align: left, text(size: 5.5pt, weight: "bold", fill: grey)[zero])
  },
  if zero-singletons {
    lq.place(xmin + label-offset, 7, align: left, text(size: 5.5pt, weight: "bold", fill: ink)[zero])
  },
)
...
#let rank-profile-comparison-grid-figure() = block(width: 100%)[
  #text(size: 8pt, weight: "bold")[The same retriever evidence produces four qualitatively different rankings]
  #text(size: 7pt)[Every panel uses the Figure 1 profiles. Panels (a)--(c) compare with one rank-1 appearance; logISR cannot use a singleton baseline, so panel (d) compares with ranks $(1,1)$.]
  #v(3pt)
  #grid(
    columns: (1fr, 1fr),
